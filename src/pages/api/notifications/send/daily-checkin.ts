...
export default async function handler(request: NextApiRequest, response: NextApiResponse) {
  ...
  try {
    ...
    console.log(
      'Sending daily check-in expires notifications',
      'lt:',
    ...
    const notifications = await prisma.notification.findMany({
      where: {
        ...
        user: {
          write_access: true,
          last_daily_claim: {
            lt: new Date(now.getTime() - (EXPIRE_MAX_HOURS - 1) * 60 * 60 * 1000),
            gt: new Date(now.getTime() - EXPIRE_MAX_HOURS * 60 * 60 * 1000),
          },
        },
      },
      select: {
    ...
    for (const notification of notifications) {
      try {
        await sendNotification(
          ...
          NOTIFICATIONS_MESSAGES.DAILY_CHECK_IN_EXPIRES
        );
    ...
    console.log(
      '[NOTIFICATIONS] Send: dailyCheckInExpires',
