...
export default async function handler(request: NextApiRequest, response: NextApiResponse) {
  ...
  try {
    ...
    for (const notification of notifications) {
      try {
        await sendNotification(notification.user.telegram_id, NOTIFICATIONS_MESSAGES.LIVES_RESET);
    ...
    console.log('[NOTIFICATIONS] Send: livesReset', notifications.length, successCount, errorCount);
